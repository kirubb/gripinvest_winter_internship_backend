...
async function create(productData) {
  let { description } = productData;

  if (!description || description.trim() === '') {
    description = generateDescription(productData);
  }
  
  const updatedProductData = { ...productData, description };
  const { name, investment_type, tenure_months, annual_yield, risk_level, min_investment, max_investment } = updatedProductData;
  ...
  return { id: result.insertId, ...updatedProductData };
}
