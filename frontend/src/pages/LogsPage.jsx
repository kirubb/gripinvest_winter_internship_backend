...
function LogsPage({ onLogout }) {
  ...
  const fetchMyLogs = async () => {
    ...
    setSearchTitle('');
    try {
      const [logsResponse, summaryResponse, errorSummaryResponse] = await Promise.all([
        apiClient.get('/logs'),
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await apiClient.post('/logs/search', { email: searchEmail });
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md">
          <h2 className="text-xl font-bold text-center mb-4">Admin Verification</h2>
          <p className="text-center text-gray-300 mb-6">You sure you're an admin right? Please enter password to enter pookie (❁´◡`❁)</p>
          ...
          <p className="text-center text-red-500 font-bold text-sm mt-6">The password is password</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <h1 className="text-3xl font-bold mb-6">{isSearching ? searchTitle : 'Your Activity Logs'}</h1>
        ...
        <div className="bg-gray-700/50 p-4 rounded-lg mb-6">
            <form onSubmit={handleSearch} className="flex items-center space-x-2">
                ...
                />
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Search</button>
                {isSearching && (
                    <button type="button" onClick={fetchMyLogs} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg">Clear Search</button>
                )}
            </form>
        </div>
        ...
        {!isSearching && summary && (
          <div className="bg-indigo-900/50 border border-indigo-700 p-4 rounded-lg mb-6">
            <h2 className="text-lg font-bold text-indigo-300">AI Activity Summary (My Logs)</h2>
        ...
        {!isSearching && errorSummary && (
          <div className="bg-red-900/50 border border-red-700 p-4 rounded-lg mb-6">
            <h2 className="text-lg font-bold text-red-300">AI Error Analysis (My Logs)</h2>
